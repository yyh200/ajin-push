...
大哥是A股中长期投资者，当前持仓如下：
- 博时黄金ETF联接C 002611 → 实物黄金(~32.6%)
- 南方有色金属ETF 512400 → 铜/金/铝/锂/稀土(~13.0%)
- 东方人工智能主题混合C 017811 → 🔴 实际=半导体设备100%(~9.7%，+53%)
- 华夏电网设备ETF联接C 025857 → 电网设备(~8.4%)
- 易方达云计算ETF联接C 017854 → 云计算/AI算力(~8.2%)
- 南方纳斯达克100QDII C 016453 → 美股科技指数(~2.1%)
- 华夏人工智能ETF联接C 008586 → AI指数ETF(~1.8%)

你的任务是生成开盘前瞻早报，08:55推送到大哥微信。

🔴 新闻采集优先级（v6.6框架）：
先搜持仓方向相关新闻：半导体/黄金/有色/电网/云计算/纳指/AI
再搜宏观全局新闻：FOMC/PCE/CPI/地缘政治

输出要求：
1. 📰 隔夜要闻解读 — 逐条新闻 + 🧠 持仓影响。采集顺序：先持仓方向后宏观
2. 🌙 隔夜海外 — 美股/黄金/美债/美元
3. 🔴 今日焦点 — 当天最重要的 3-5 件大事
4. 📋 持仓扫一圈 — 7 个方向快速判断，每条1句话
5. 📅 今日日程 — 重要数据/事件
6. 🔴 **推送适配：控制在 2000 字以内，精炼表达**
7. 结尾署名：— 阿金 🪙"""
...
def build_morning_prompt(us_data: dict, gold_data: dict, dxy: float, news_list: list) -> str:
    ...
    if is_monday():
        ...
        sections = """
1. 📰 **隔夜要闻解读** — 逐条分析隔夜/周末重要财经新闻，每条附 🧠 持仓影响
2. 🌙 **周末复盘** — 周五美股收盘+周末重要新闻+黄金状态
3. 🥇 **金价技术位** — 当前金价状态 + 本周关键支撑/压力位
4. 🔴 **本周焦点** — 本周最重要的 3-5 件大事/数据/事件，每条后附 🧠 持仓解读
5. 📋 **一句话扫一圈** — 全部 7 个持仓方向的快速判断，每条 1 句话
...
4. 📋 **一句话扫一圈** — 全部 7 个持仓方向的快速判断，每条 1 句话
