...
- 博时黄金ETF联接C 002611 → 实物黄金(~33.2%)，保底层
- 东方人工智能主题混合C 017811 → 🔴 实际=半导体设备100%（前十大集中度86%），进攻层(~20.3%)
- 华夏电网设备ETF联接C 025857 → 电网设备(~14.1%)，进攻层
- 易方达云计算ETF联接C 017854 → 云计算/AI算力(~7.8%)，进攻层
- 华夏人工智能ETF联接C 008586 → AI指数ETF(~4.5%)，进攻层
- 华夏中证5G通信ETF联接C 008087 → ⚠️ 实际=CPO(~50%)+PCB(~30%)，进攻层(~4.1%)
- 南方纳斯达克100QDII C 016453 → 美股科技指数(~2.2%)，进攻层
- 已清仓：南方有色金属ETF 512400（❌ 6/26清仓）
- 现金 ~13.8%

🔴 穿透后真实结构（必须识别）：
  表面7只基金 → 实际只有3个方向：黄金33% + 科技链49%（半导体+云+AI+5G+纳指）+ 电网14%
