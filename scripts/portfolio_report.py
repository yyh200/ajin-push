...
使用「投资罗盘 v6.0」流水线框架执行分析：Phase 0预检 → Phase 1数据采集 → Phase 2七维诊断（D1-D7）→ Phase 3组合审查 → Phase 4建议 → Phase 5自批判。每个Phase不可跳过，每个方向必须牛熊双面。
...
4. 必须包含 D7 政策事件日历
5. 必须包含 Phase 5 自我批判（含确认偏误检查）
6. 报告第一部分必须是财经新闻解读，逐条标注 🧠 持仓影响
7. 结尾署名：— 阿金 🪙

七维诊断 + 双面牛熊 + D7政策日历 + 确认偏误检查。一个都不准少。"""
